...
func TestBlockHeader(t *testing.T) {
	...
	bh := NewLegacyBlockHeader(1, &hash, &merkleHash, bits, nonce)

	// Ensure we get the same data back out.
	if !bh.PrevBlock.IsEqual(&hash) {
		t.Errorf("NewLegacyBlockHeader: wrong prev hash - got %v, want %v",
			spew.Sprint(bh.PrevBlock), spew.Sprint(hash))
	}
	if !bh.MerkleRoot.IsEqual(&merkleHash) {
		t.Errorf("NewLegacyBlockHeader: wrong merkle root - got %v, want %v",
			spew.Sprint(bh.MerkleRoot), spew.Sprint(merkleHash))
	}
	if bh.Bits != bits {
		t.Errorf("NewLegacyBlockHeader: wrong bits - got %v, want %v",
			bh.Bits, bits)
	}
	if LowUint32FromUint256(bh.Nonce) != nonce {
		t.Errorf("NewLegacyBlockHeader: wrong nonce - got %v, want %v",
			bh.Nonce, nonce)
	}
}
