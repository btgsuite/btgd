...
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the btcd
  ...
  in the btcd home directory (which is typically `%LOCALAPPDATA%\Btgd` on
  Windows and `~/.btgd` on POSIX-like OSes)
...
|Method|stop|
|Parameters|None|
|Description|Shutdown btgd.|
|Returns|`"btgd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
func main() {
	...
	// have one.
	btcdHomeDir := btgutil.AppDataDir("btgd", false)
...
// generated by btcd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/btgd/appdata/rpc.cert');
